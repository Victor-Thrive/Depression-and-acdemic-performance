...
# Helper function to calculate mode (returns the most frequent value)
get_mode <- function(x) {
  ux <- unique(x[x != 0])  # Exclude 0 values from mode calculation
...
# data setup -------------------------------------------------------------------
depression  <- read.csv("data/final.csv", stringsAsFactors = TRUE) |>
  # Replace 0s with mode for each column in f1:f3 and restrict values to 1:5
  mutate(across(f1:f3, ~ ifelse(. == 0, get_mode(.), .))) |>
  mutate(across(f1:f3, ~ factor(.x, 
                                  levels = 1:5, 
...
set.seed(1)
mutagen_split <- initial_split(depression)
